...
def update_s3(dataset_id, resource_name=None, resource_title=None, owner_org=None, resource_s3=None, notes=None, extras=None):
    """
    Update an S3 dataset and its associated metadata.
    ...
    resource_s3 : Optional[str]
        The S3 URL of the resource.
    notes : Optional[str]
    ...
    try:
        ...
        # Update the resource if S3 URL is provided
        if resource_s3:
            resource = next((res for res in updated_dataset['resources'] if res['format'] == 's3'), None)
            if resource:
                resource['url'] = resource_s3
                resource['description'] = f"Resource pointing to {resource_s3}"
