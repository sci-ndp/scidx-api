    ...
    for rule in rules:
        try:
            if "IF" in rule and "THEN" in rule:
                ...
                # Apply ELSE action if present and row is still NaN after THEN
                if else_part:
                    ...
                    # Apply ELSE only where the column remains NaN after the window filter and is not due to window insufficiency
